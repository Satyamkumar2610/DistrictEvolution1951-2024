...
class LineageEvent(BaseModel):
    ...
    parent_name: str | None = Field(default=None, description="Human-readable parent name")
    ...
    coverage_ratios: dict[str, float] = Field(
        default_factory=dict, description="Area proportion per child (should sum to ~1.0)"
    )
    legal_reference: str | None = Field(default=None, description="Gazette notification reference")
